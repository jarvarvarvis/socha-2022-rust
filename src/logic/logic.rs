...
impl Logic {
    ...
    fn process_move_request(&mut self, protocol_manager: &mut ProtocolManager) {
        ...
        match calculated_move {
            Some(sent_move) => {
                ...
                let message = ClientSideMessage::Move {
                    sent_move,
                    room_id
                };
                match protocol_manager.send_client_side_message(message) {
                    Ok(_) => {},
                    Err(error) => {
    ...
    pub fn process_server_side_message(&mut self, protocol_manager: &mut ProtocolManager, message: ServerSideMessage) {
        match message {
